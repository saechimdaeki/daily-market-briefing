...
def extract_tickers_from_news(headlines):
    ...
    5. 펀드/운용사('한화운용', 'KB자산운용' 등)는 제외. 상장된 **개별 주식**만 추출.
    6. 미국 주식은 티커만(예: AAPL), 한국 주식은 반드시 .KS(코스피) 또는 .KQ(코스닥) 접미사 사용(예: 005930.KS, 000660.KS).
    7. 뉴스 본문에 티커가 잘못 표기되어 있어도(예: 삼전닉스에 005935 표기) 위 규칙을 우선 적용해 **올바른 티커로 보정**할 것.
